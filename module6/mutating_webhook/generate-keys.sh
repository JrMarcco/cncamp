...
[alt_names]
DNS.1 = mutating-webhook.demo.svc
EOF
...
openssl genrsa -out mutating-webhook-server-tls.key 2048
# Generate a Certificate Signing Request (CSR) for the private key, and sign it with the private key of the CA.
openssl req -new -key mutating-webhook-server-tls.key -subj "/CN=mutating-webhook.demo.svc" -config server.conf \
    | openssl x509 -req -CA ca.crt -CAkey ca.key -CAcreateserial -out mutating-webhook-server-tls.crt -extensions v3_req -extfile server.conf